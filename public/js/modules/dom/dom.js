...
import { newTaskDeclareElements, declareDropdownContent, createTagTable, appendTaskChildren, removePlaceholderIfTaskExists, saveTasksToAppData } from "../../export.js";
...
function populateYearDropdown(currentYear, yearDropdown) {
    let yearOptions = yearDropdown.getElementsByTagName('option');
    let selectedYearValue = localStorage.getItem('selectedYearValue');

    for (let i = 1; i < options.length; i++) {
        yearOptions[i].value = currentYear + (i - 1);
        yearOptions[i].id = `optionYear-${currentYear + (i - 1)}`;
        yearOptions[i].textContent = currentYear + (i - 1);
    }
...
function createTask(taskCount, taskInput, tagsInput) {
    taskCount++;

    let { taskCount, taskCellDiv, taskNumberBoxDiv, taskTopRightBarDiv, taskMainDiv, taskTagRowDiv, taskInfoBarDiv, selectedMonth, selectedDay, selectedHour, selectedMinute, selectedAMPM, selectedYear, selectedPriority, taskColumnPlaceholder, taskTimeSkipCheckbox, } = newTaskDeclareElements(taskCount);
    ...
    let { selectedMonthNumberInt, selectedYearNumberInt, selectedDayNumberInt } = declareDropdownContent();
